        ...
        <div class="lg:col-span-1">
          ...
          <ul class="space-y-3">
            {rooms.map((r) => (
              <li
                key={r.id}
                class="p-4 border border-gray-700 rounded-xl flex justify-between items-center transition-all duration-200 bg-gray-800 hover:bg-gray-750 hover:border-gray-600"
              >
                <a href={`/rooms/${r.id}`} class="flex-grow group">
                    <span class="font-medium text-lg text-white block group-hover:text-indigo-400 transition-colors">{r.name}</span>
